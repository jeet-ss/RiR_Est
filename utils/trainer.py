...
class Trainer:
    ...
    def train_step_Geo(self, rir_features, geo_labels, batch_idx):
        # reset zero grad
    ...
    def train_step_reCoeff(self, rir_features, coeff_l, batch_idx):
        # reset zero grad
        self.optimizer.zero_grad()
        #
    ...
    def train_step_Link(self, rir_features,room_geo, coeff_l, batch_idx):
        batch_size = rir_features.size()[0]
        # reset zero grad
        self.optimizer.zero_grad()
        #
        pred = self._model(rir_features, room_geo).reshape(batch_size,3,3)
        #print("in train", pred.squeeze(axis=1).size(),room_geo.unsqueeze(axis=1), coeff_l.reshape(3,2).size() )
        # calculate loss
        labels = torch.cat((room_geo.reshape(batch_size, 3, 1), coeff_l.reshape(batch_size,3,2)), dim=2)
    ...
    def val_test_step_Geo(self, rir_features, geo_labels, batch_idx):
        # predict
        pred = self._model(rir_features.unsqueeze(axis=1))
        # loss
        loss = self.criterion(pred, geo_labels)
    ...
    def val_test_step_reCoeff(self, rir_features, coeff_l, batch_idx):
        # predict
        pred = self._model(rir_features)
        # loss
        loss = self.criterion(pred, coeff_l)
    ...
    def val_test_step_Link(self, rir_features, room_geo, coeff_l, batch_idx):
        batch_size = rir_features.size()[0]
        # predict
        pred = self._model(rir_features, room_geo).reshape(batch_size,3,3)
    ...
    def train_epoch(self):
        ...
        for idx, batches in enumerate(self.train_batches):
            ...
            #loss_step = self.train_step_Geo(rir_f, geo_l, idx)
    ...
    def val_epoch(self):
        ...
        with torch.no_grad():
            for idx, batches in enumerate(self.val_batches):
                ...
                #loss_step, pred = self.val_test_step_Geo(rir_f, geo_l, idx)
